...
AI가 생성한 HTML 산출물을 게시하고, 사람의 피드백을 모아, 다시 coding agent에게 전달하세요.

Agent는 이제 HTML을 잘 만듭니다 — 덱, 리포트, 프로토타입. PreApp은 마지막 구간을 책임집니다: **공유하고, 리뷰받고, 코멘트를 다음 agent 실행에 넘기기.**

```text
agent publish → 공유 링크 → 사람이 코멘트 → agent가 피드백 읽기 → agent가 v2 publish
```
...
반환된 `reviewLink`를 공유하면 누구나 가입 없이 코멘트할 수 있습니다(텍스트 선택·이미지 클릭 주석). 그다음:

```bash
preapp feedback get q3-strategy --format markdown
```

Agent는 **Agent Fix Brief** — 정확한 위치 지정자가 붙은 번호별 코멘트 — 를 받아 수정하고 v2를 게시합니다. **링크는 그대로**입니다.
...
Agent는 HTML을 만들 수 있지만, 그것을 사람 앞에 놓는 일은 여전히 번거롭습니다:
...
- **View / Review 이중 링크** — 깔끔한 열람과 가벼운 주석을 분리, 권한도 별도.
- **문제 지점에 코멘트** — 텍스트 선택 또는 이미지 클릭; 앵커·덱 전체 코멘트도 지원.
...
`preapp feedback get`의 출력은 의도적으로 게이트 지시로 끝납니다: agent는 **코멘트를 사람에게 전달하고 멈춰야 합니다** — 조용한 자동 수정 금지. 사람이 어떤 코멘트를 반영할지 정한 뒤에야 agent가 수정하고 다시 게시합니다. 이는 제한이 아니라 제품 결정입니다; [docs/cli.md](docs/cli.md#two-stage-feedback-gate) 참고.
...
- 리뷰어 코멘트는 agent에게 **신뢰할 수 없는 데이터**입니다(prompt injection 방어) — skill과 문서 모두 내용으로만 다루고 지시로 취급하지 않도록 정합니다.
